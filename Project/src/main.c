...
void ChangeDeviceRGBStatus(bool _sw, uint8_t _br, uint8_t _r, uint8_t _g, uint8_t _b, uint8_t _ring) {
  if(!_sw) {
    _br = 0;
  } else if(_br > 100) {
    _br = 100;
...
// Immediately change rgb
void ChangeDeviceRGB(uint32_t _rgb, uint8_t _ring) {
#if defined(XRAINBOW) || defined(XMIRAGE)
  uint8_t _r, _g, _b;
  _b = (_rgb & 0xff);
  _rgb >>= 8;
...
#ifdef RING_INDIVIDUAL_COLOR
  uint8_t r_index = (_ring == RING_ID_ALL ? 0 : _ring - 1);
  ChangeDeviceRGBStatus(TRUE, RINGST_Bright(r_index), _r, _g, _b, _ring);
#else
  ChangeDeviceRGBStatus(TRUE, DEVST_Bright, _r, _g, _b, _ring);
#endif
#endif  
}
...
bool SetDeviceRGB(uint8_t _r, uint8_t _g, uint8_t _b, uint8_t _br,uint8_t _ring) {
  ...
  uint8_t r_index = (_ring == RING_ID_ALL ? 0 : _ring - 1);
  if( _r != RINGST_R(r_index) || _g != RINGST_G(r_index) || _b != RINGST_B(r_index) ) {
#ifdef GRADUAL_RGB    
    // Smoothly change RGBW - set parameters
    delay_from[DELAY_TIM_RGB] = cf_makeColorValue(RINGST_R(r_index), RINGST_G(r_index), RINGST_B(r_index));
...
#else    
    ChangeDeviceRGB(newValue, _ring);
#endif
...
bool SetDeviceHue(bool _sw, uint8_t _br, uint8_t _r, uint8_t _g, uint8_t _b, uint8_t _ring) {
  ...
  return SetDeviceRGB(_r, _g, _b, _ring);
...
void StartDeviceColorFade(bool _init, bool _fast) {
  ...
  delay_handler[DELAY_TIM_RGB] = ChangeDeviceRGB;
